...
ASM_MAIN_SRC = $(wildcard Assembler/main.cpp)
ASM_NO_MAIN_SRC = $(filter-out $(ASM_MAIN_SRC), $(ASM_SRC))
SUBS_ASM_SRC = $(wildcard Assembler/SubsidiaryFunctionsAssembler.cpp)
HASH_SRC = Hash.cpp
NCURSES_SRC = Ncurses/NcursesWin.cpp
COMMANDS_SRC = Commands.cpp

CFLAGS = -D_DEBUG -ggdb3 -g -std=c++17 -O0 -Wall -Wextra -Weffc++ -Wc++14-compat -lncurses -Wmissing-declarations -Wcast-align -Wcast-qual -Wchar-subscripts -Wconversion -Wctor-dtor-privacy -Wempty-body -Wfloat-equal -Wformat-nonliteral -Wformat-security -Wformat=2 -Winline -Wnon-virtual-dtor -Woverloaded-virtual -Wpacked -Wpointer-arith -Winit-self -Wredundant-decls -Wshadow -Wsign-conversion -Wsign-promo -Wstrict-overflow=2 -Wsuggest-override -Wswitch-default -Wswitch-enum -Wundef -Wunreachable-code -Wunused -Wvariadic-macros -Wno-missing-field-initializers -Wno-narrowing -Wno-old-style-cast -Wno-varargs -Wstack-protector -fcheck-new -fsized-deallocation -fstack-protector -fstrict-overflow -fno-omit-frame-pointer -Wlarger-than=8192 -fPIE -Werror=vla -fsanitize=address,alignment,bool,bounds,enum,float-cast-overflow,float-divide-by-zero,integer-divide-by-zero,nonnull-attribute,null,return,returns-nonnull-attribute,shift,signed-integer-overflow,undefined,unreachable,vla-bound,vptr -I. -IAssembler -IProcessor -IDisasassembler -IFileOperations -IStackFunctions -IAllLogFile -IParse -IProcessor/ProcessorFunctions -IProcessor/Calc -ICalc -INcurses

asm: $(ASM_SRC)
	@$(COMP) $(CFLAGS) -D_Assembler $(ASM_SRC) $(FILEOP_SRC) $(PARSE_SRC) $(ALLLOG_SRC) $(STACK_SRC) $(PROCFUNC_SRC) $(HASH_SRC) $(COMMANDS_SRC) -o asm.out
	@rm -rf *.dSYM

proc: $(PROC_SRC)
	@$(COMP) $(CFLAGS) -D_Processor $(PROC_SRC) $(FILEOP_SRC) $(PARSE_SRC) $(CALC_SRC) $(STACK_SRC) $(PROCFUNC_SRC) $(ALLLOG_SRC) $(NCURSES_SRC) $(COMMANDS_SRC) -o proc.out
	@rm -rf *.dSYM

disasm: $(DISASM_SRC)
	@$(COMP) $(CFLAGS) -D_Assembler $(DISASM_SRC) $(FILEOP_SRC) $(ASM_NO_MAIN_SRC) $(PARSE_SRC) $(STACK_SRC) $(DOFILE_SRC) $(ALLLOG_SRC) $(CALC_SRC) $(PROCFUNC_SRC) -o disasm.out
	@rm -rf *.dSYM
